...
class NDEF_PPO(object):
        ...
        with tf.name_scope('perform/'):
            ...
            sample_ = tf.concat([
                tf.cast(output.policy[ACT['DECISION']].sample()[
                    :, :, None], tf.float32),
                output.policy[ACT['OFF_DASH']].sample()], axis=2)
            mode_ = tf.concat([
                tf.cast(output.policy[ACT['DECISION']].mode()[
                        :, :, None], tf.float32),
                output.policy[ACT['OFF_DASH']].mode()], axis=2)
            action = tf.where(
    ...
    def _initialize_policy(self):
        ...
        if decision_shape != () or off_dash_shape != (12,):
    ...
    def _update_step(self, sequence):
        ...
        """
        observ, action, DECISION, OFF_DASH, reward, advantage = sequence[
            'sequence']
    ...
    def _policy_loss(
        ...
        with tf.name_scope('policy_loss'):
            ...
            policy_gradient = (off_deci_grad + off_dash_grad) / 2.0
            ...
            # NOTE: weighted sum maybe
            kl = (off_deci_kl + off_dash_kl) / 2.0
            ...
            entropy = (get_entropy(ACT['DECISION']) +
