...
class Discriminator(object):
    ...
    def __init__(self, config, env):
        ...
        with tf.variable_scope('Discriminator') as scope:
            ...
            self._expert_s = tf.placeholder(dtype=tf.float32, shape=[None]+list(env.observation_space.shape))
            self._agent_s = tf.placeholder(dtype=tf.float32, shape=[None]+list(env.observation_space.shape))
            real_score = config.d_network(self._expert_s)
            fake_score = config.d_network(self._agent_s)
            em_distance = real_score-fake_score
