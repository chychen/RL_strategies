...
class BBallNDefBSpeedEnv(gym.Env):
    ...
    def step(self, action):
        ...
        ball_pass_vel = action[ACTION_LOOKUP['BALL']]
        ...
        if not self.states.done and decision == DESICION_LOOKUP['SHOOT']:
            ...
            if self.states.ball_handler_idx is None:
                self.states.update_status(
                    done=False, status=STATUS_LOOKUP['SHOOT'])
            elif self.states.ball_position[0] >= self.court_length or self.states.ball_position[0] < self.court_length / 2 or self.states.ball_position[1] >= self.court_width or self.states.ball_position[1] < 0.0:
                # OOB and shoot
                self.states.update_status(
                    done=False, status=STATUS_LOOKUP['SHOOT'])
            elif self.any_defense_close_ball_handler():
                self.states.update_status(
                    done=False, status=STATUS_LOOKUP['SHOOT'])
            elif self.not_in_3pt_line():
                self.states.update_status(
                    done=False, status=STATUS_LOOKUP['SHOOT'])
            else:
                self.states.update_status(
    ...
    def _set_action_space(self):
        ...
        return tools.NDefActTuple((
            ...
            # ball dir
            spaces.Box(
                low=-self.ball_max_speed, high=self.ball_max_speed, shape=(2,), dtype=np.float32
            ),
    ...
    def _calculate_reward(self):
        ...
        # TODO if too close, get 0
        def2ball_lens[np.where(def2ball_lens < self.eff_def_radius)] = 0
    ...
    def any_defense_close_ball_handler(self):
        if self.states.ball_handler_idx is not None:
            # distances between all defenders to ball handler
            def2ball_vecs = self.states.ball_position - self.states.defense_positions
            def2ball_lens = length(def2ball_vecs, axis=1)
    ...
    def not_in_3pt_line(self):
        ball2basket_vec = self.right_basket_pos - self.states.ball_position
        ball2basket_len = length(ball2basket_vec, axis=0)
        return ball2basket_len > self.three_point_distance
